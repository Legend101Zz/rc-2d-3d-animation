let tween; // Declare the tween variable outside the init() function

// window.onload = init;

const btn = document.getElementById("3d");
console.log(btn);
// function check() {
//   console.log("hit");
// }
...
function init() {
  ...
  root.renderer.setClearColor(0x00000);
  ...
  const dom = root.renderer.domElement;
  console.log(dom);
  ...
  root.scene.add(cubeMesh);
  // Add the cube to the scene

  // Animate the flip and scale down effect
...
function createTextAnimation() {
  var geometry = generateTextGeometry("ROCKET.CHAT", {
    size: 14,
    height: 0,
    font: "droid sans",
    weight: "bold",
  ...
  // var textAnimation = new TextAnimation(geometry);
...
function TextAnimation(textGeometry) {
  ...
  //   if (scope._animationProgress >= 1) {
  //     scope.dispatchEvent({ type: "animationEnd" });
  //   }
  // }
...
console.log(THREE.EventDispatcher.prototype.apply(TextAnimation.prototype));
...
THREERoot.prototype = {
  ...
  update: function () {
    ...
    if (animationEnded && !flipAnimation) {
      flipAnimation = true;
